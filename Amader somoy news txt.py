...
def get_tittle_and_content(url):
    ...
    title_div = soup.find("div",{"class":"w3-row w3-margin-left w3-margin-right"})
    newsTitle = title_div.find_all("h1")[0].getText()
    print(newsTitle)
    det_div = soup.find("div",{"class":"w3-col l8"})
    news_det = det_div.find_all("span")[0].getText()
    print(news_det)
